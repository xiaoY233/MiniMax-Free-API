...
async function removeConversation(convId: string, token: string) {
  try {
    ...
    // Agent API 可能不支持删除会话,静默失败
    logger.warn(`Failed to remove conversation: ${err.message}`);
  }
}
    ...
    const parser = createParser((event) => {
      try {
        ...
        if (eventName == "message_result" && messageResult) {
          const { chatID, chat_id: agentChatId, msgID, msg_id: agentMsgId, isEnd, content, extra } = messageResult;
          const finalChatId = agentChatId || chatID || chat_id;
          const finalMsgId = agentMsgId || msgID || msg_id;

          if (!data.id && finalChatId) data.id = finalChatId;
          if (message_id_required && !data.message_id && finalMsgId) data.message_id = finalMsgId;
...
function createTransStream(model: string, stream: any, endCallback?: Function) {
  ...
  const parser = createParser((event) => {
    try {
      ...
      const { messageResult } = _data || {};
      if (eventName == "message_result" && messageResult) {
        const { chatID, chat_id: agentChatId, isEnd, content: text, extra } = messageResult;
        const finalChatId = agentChatId || chatID;

        if (isEnd !== 0 && !text) return;
        if (!convId && finalChatId) convId = finalChatId;
        ...
        if (isEnd === 0) {
          ...
          endCallback && endCallback(finalChatId);
        }
      }
    } catch (err) {
      logger.error(err);
