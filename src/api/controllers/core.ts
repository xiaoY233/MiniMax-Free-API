...
async function requestStream(
  ...
  // Agent API 使用 JSON 格式，不是 FormData
  let dataJson = JSON.stringify(data || {});
  let requestData: any;
  let contentType = "application/json";

  // 如果有音频数据，使用 FormData
  if (data.voiceBytes && _.isBuffer(data.voiceBytes)) {
    ...
    // 使用 JSON 格式
    requestData = Buffer.from(dataJson, "utf8");
  }
  ...
  // 发送数据
  if (requestData.getBuffer) {
    stream.end(requestData.getBuffer());
  } else {
    stream.end(requestData);
  }
